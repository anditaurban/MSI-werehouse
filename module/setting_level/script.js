  ...
  <tr class="flex flex-col sm:table-row border rounded sm:rounded-none mb-4 sm:mb-0 shadow-sm sm:shadow-none transition hover:bg-gray-50">  
    ...
    <td class="px-6 py-4 text-sm text-gray-700 flex justify-between sm:table-cell">
      ...
      <div class="dropdown-menu hidden fixed w-48 bg-white border rounded shadow z-50 text-sm">
       <button onclick="event.stopPropagation(); handleEdit(${item.level_id}, '${item.level.replace(/'/g, "\\'")}')" class="block w-full text-left px-4 py-2 hover:bg-gray-100">✏️ Edit</button>
        <button onclick="event.stopPropagation(); handleDelete(${item.level_id})" class="block w-full text-left px-4 py-2 hover:bg-gray-100 text-red-600">
          🗑 Delete User
        </button>
      </div>
    </td>
...
<form id="dataform" class="space-y-2">
  ...
  <input id="formLevel" name="level" type="text" class="form-control w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">

  <label for="formDesc" class="block text-sm font-medium text-gray-700 dark:text-gray-200 text-left">Description</label>
